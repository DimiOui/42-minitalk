...
/*   Updated: 2022/01/26 12:22:00 by dpaccagn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	The handler will print every binary sent into a character
// and send back a signal to the client
//	to acknowledge it has been clearly received
//	The siginfo structure is passed as a second parameter to
// a user signal handler function whenever the SA_SIGINFO flag was
//	specified. The void *context points to the structure of
// a signal that was saved (unsused here)
static void	my_handler_to_char(int signum, siginfo_t *siginfo, void *context)
{
	...
	if (g_char.bits < 0)
	{
		if (!g_char.c)
		{
			...
			g_char.str = ft_strjoin_char(g_char.str, g_char.c);
		struct_init();
	}
	...
	//add SIGUSR1 to our sigaction struct so it's
	// recognized (returns -1 on error)
	//add SIGUSR2 to our sigaction struct so it's
	// recognized (returns -1 on error)
	// when SA_SIGINFO is specified in sa_flags :
	// sa_mask specifies a mask of signals which should be blocked
	// sa_sigaction specifies the signal handling function for signum
	// sa_flags specifies a set of flags which modify
	// the behavior of the signal
	// sigaction takes the signum, the sigaction in use,
	// and old sigaction (unused here)
